...
const PrehistoryPage: NextPage<Props> = ({periods, topics}) => {
    const classes = useStyles();
    const title = periods.filter(x => x.type === HistoryPeriod.prehistory)[0].title;

    return (
        <DefaultLayout title={title}>
            <HistoryPeriodTabs selected={HistoryPeriod.prehistory} periods={periods}/>
...
export const getServerSideProps: GetServerSideProps<Props> = async ({req, res}: GetServerSidePropsContext) => {
    ...
    const topics = await HistoryApi.getTopics(HistoryPeriod.prehistory, token);
