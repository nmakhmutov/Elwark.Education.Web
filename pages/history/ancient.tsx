...
const AncientPage: NextPage<Props> = ({periods, topics}) => {
    ...
    const title = periods.filter(x => x.type === 'ancient')[0].title;

    return (
        <DefaultLayout title={title}>
            <HistoryPeriodTabs selected={'ancient'} periods={periods}/>
...
export const getServerSideProps: GetServerSideProps<Props> = async ({req, res}: GetServerSidePropsContext) => {
    ...
    const topics = await HistoryApi.getTopics('ancient', token);
